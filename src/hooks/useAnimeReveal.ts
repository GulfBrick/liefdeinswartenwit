...
/**
 * useAnimeRevealMultiple — attach anime reveal to multiple refs at once.
 * Returns an array of refs. Each triggers independently with progressive delay.
 */
export function useAnimeRevealMultiple(
  count: number,
  baseOptions: AnimeRevealOptions = {}
  ...
  useEffect(() => {
    ...
    const { preset = 'fadeUp', duration = 900, staggerDelay = 200 } = baseOptions;
    const props = presetMap[preset];
    const cleanups: (() => void)[] = [];
